...
class ScoringService(object):
    ...
    def get_model(cls):
        ...
        if cls.model == None:
            model = Sequential()
            conv = Conv2D
            model.add(conv(input_shape=(1, 100, 121),
            ...
            model.add(Lambda(lambda a: K.expand_dims(a[:, 0], -1) + a[:, 1:] - K.mean(a[:, 1:], keepdims=True),
                             output_shape=(3,)))
            model.load_weights(os.path.join(model_path, "dqn_trend-following-v0_cnn_weights_model_weights.h5"))
            cls.model=model
        return cls.model
